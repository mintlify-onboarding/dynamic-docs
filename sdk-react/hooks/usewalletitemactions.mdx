...
description: "A hook with utility functions to sign with a specific wallet. The hook needs to be initialized within a child of `DynamicContextProvier`"
---
...
const SignWithMetaMaskButton = () => {
  ...
  return (
    <button onClick={() => openWallet("metamask")}>Sign with MetaMask</button>
  );
};
```
